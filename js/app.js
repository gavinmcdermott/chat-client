...
var populateMessages = function(data) {
  $.each(data.results, function(which, message) {
    $('.message_container').append(renderMessage(message));
  });
};
...
var postMessage = function () {
  var theMessage = {username: $('.name').val(), text: $('.writeup').val(), roomname: currentRoom};
...
var renderMessage = function(message) {
  ...
  var $node = $(
    "<div class='message'>"+
      "<span class='user' data-username="+message.username+">"
        + message.username + 
      "</span>" +
      "<span class='text' data-username="+message.username+">"
        + message.text +
      "</span>" +
    "</div>");
  return $node;
